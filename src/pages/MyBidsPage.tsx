...
export function MyBidsPage() {
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      <p className="mt-2 text-sm text-muted">Only confirmed payments appear as live bids on the leaderboard.</p>
      <div className="mt-6">
        {loading ? (
          ...
          <ul className="space-y-2">
            {bids.map((bid) => (
              <li key={bid.id} className="flex items-center gap-3 rounded-md border border-line bg-white px-4 py-3">
                <ProductLogo
                  src={bid.product.logoUrl}
                  name={bid.product.name}
                  siteUrl={bid.product.url}
                />
                ...
                <div className="text-right">
                  ...
                  <div className={`text-xs ${bid.status === "succeeded" ? "text-emerald-600" : bid.status === "failed" ? "text-danger" : "text-muted"}`}>
                    {bid.status}
                  </div>
                </div>
              </li>
